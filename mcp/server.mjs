 ...
 *
 * Exposed tools:
 *   - zoek_kennisbank        search the knowledge base (full text)
 *   - lees_artikel           read one article as Markdown
 *   - lijst_artikelen        list all articles with metadata
 ...
 *
 * Data source: ../kennisbank/index.json (generated by kb-build.mjs),
 * re-read from disk at most once per minute so deploys are picked up
 * without restarting.
 *
...
const SERVER_INFO = { name: 'dekunstvanwerken', version: '1.0.0' };
const PROTOCOL_VERSIONS = ['2025-06-18', '2025-03-26', '2024-11-05'];

// ---------------------------------------------------------------- data
let kbCache = { at: 0, data: null };
function loadKb() {
  const now = Date.now();
  if (kbCache.data && now - kbCache.at < 60_000) return kbCache.data;
  try {
    const raw = readFileSync(join(ROOT, 'kennisbank', 'index.json'), 'utf8');
    kbCache = { at: now, data: JSON.parse(raw) };
  } catch (e) {
    if (!kbCache.data) kbCache = { at: now, data: { articles: [] } };
  }
  return kbCache.data;
}

function readArticleMarkdown(id) {
  if (!/^[a-z0-9-]+$/.test(id)) return null; // guard against path tricks
  try {
    return readFileSync(join(ROOT, 'kennisbank', `${id}.md`), 'utf8');
  } catch {
    return null;
  }
}
...
const TOOLS = [
  {
    ...
    description: 'Doorzoek de kennisbank van de Kunst van Werken (artikelen over werk, gezondheid, focus en re-integratie). Zoekt in titel, samenvatting, tags en volledige tekst.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Zoekterm(en), bv. "yerkes-dodson" of "re-integratie"' },
      },
      required: ['query'],
    },
    handler: ({ query }) => {
      ...
      const hits = loadKb().articles.filter((a) => {
        const hay = `${a.title}\n${a.summary}\n${(a.tags || []).join(' ')}\n${a.content}`.toLowerCase();
        return q.split(/\s+/).every((term) => hay.includes(term));
      }).map((a) => {
        const idx = a.content.toLowerCase().indexOf(q.split(/\s+/)[0]);
        const snippet = idx >= 0
          ? a.content.slice(Math.max(0, idx - 80), idx + 160).replace(/\s+/g, ' ')
          : a.summary;
        return { id: a.id, title: a.title, summary: a.summary, tags: a.tags,
          url: a.url, snippet: `…${snippet}…` };
      });
      return { aantal: hits.length, resultaten: hits };
    },
  },
  {
    name: 'lees_artikel',
    description: 'Lees één kennisbank-artikel volledig, als Markdown. Gebruik het id uit zoek_kennisbank of lijst_artikelen.',
    inputSchema: {
      type: 'object',
      properties: { id: { type: 'string', description: 'Artikel-id (slug), bv. "arousal-performance-curve"' } },
      required: ['id'],
    },
    handler: ({ id }) => {
      const md = readArticleMarkdown(String(id || ''));
      if (!md) throw new Error(`Onbekend artikel: ${id}. Gebruik lijst_artikelen voor geldige id's.`);
      return md;
    },
  },
  {
    name: 'lijst_artikelen',
    description: 'Alle kennisbank-artikelen met id, titel, samenvatting, tags en URL.',
    inputSchema: { type: 'object', properties: {} },
    handler: () => ({
      aantal: loadKb().articles.length,
      artikelen: loadKb().articles.map((a) => ({
        id: a.id, title: a.title, summary: a.summary, tags: a.tags,
        lang: a.lang, date: a.date, url: a.url,
      })),
    }),
  },
  {
  ...
  switch (method) {
    case 'initialize': {
      ...
      return rpcResult(id, {
        ...
        instructions: 'MCP-server van de Kunst van Werken (arbodienstverlening, NL). Gebruik zoek_kennisbank/lees_artikel voor de kennisbank, bereken_opbouwschema voor re-integratie-opbouwschema\'s, en faq/bedrijfsinfo voor algemene informatie.',
      });
    }
    case 'ping':
      return rpcResult(id, {});
...
const server = createServer((req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/healthz') {
    ...
    return res.end(JSON.stringify({ ok: true, server: SERVER_INFO, articles: loadKb().articles.length }));
  }
